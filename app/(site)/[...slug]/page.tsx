...
export async function generateStaticParams() {
  ...
  return pages.map((page) => {
    const slugArray = page.parentSlug
...
export default async function Page({ params }: PageProps) {
  // Convert slug array to string (last segment is the actual page slug)
  const pageSlug = params.slug[params.slug.length - 1]
  ...
  const portableTextComponents = {
    ...
    block: {
      ...
      blockquote: ({ children }: any) => (
        <blockquote className="border-l-4 border-primary pl-6 my-6 italic text-lg text-gray-700">
          {children}
        </blockquote>
      ),
      normal: ({ children }: any) => (
    ...
    marks: {
      ...
      code: ({ children }: any) => (
        <code className="bg-gray-100 text-secondary px-2 py-1 rounded text-sm font-mono">
          {children}
        </code>
      ),
    },
